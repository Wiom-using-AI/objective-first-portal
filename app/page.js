  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">Objective conduction across Wiom — are objectives surviving handoffs?</p>
      </div>
      ...
      {m.totalScored > 0 && (
        <div className="bg-white rounded-lg border border-gray-200 p-5">
          <h2 className="text-sm font-semibold text-gray-900 uppercase tracking-wide mb-4">Alignment Breakdown</h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-lg border border-gray-200 p-5">
          <h2 className="text-sm font-semibold text-gray-900 uppercase tracking-wide mb-4">Who Has Submitted</h2>
          {m.recentSubmissions.length === 0 ? (
            <p className="text-sm text-gray-500">No submissions yet. <a href="/submit" className="text-[#E91E63] underline">Start here.</a></p>
          ) : (
                ...
                <div key={i} className="flex items-center justify-between py-2 border-b border-gray-50 last:border-0">
                  <div>
                    ...
                    {s.submitter_function && (
                      <span className="ml-2 text-xs bg-pink-50 text-[#E91E63] px-2 py-0.5 rounded">{s.submitter_function}</span>
                    )}
                  </div>
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-5">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-sm font-semibold text-gray-900 uppercase tracking-wide">Recent Objective Notes</h2>
            <a href="/projects" className="text-xs text-[#E91E63] hover:underline">View all</a>
          </div>
          {m.recentNotes.length === 0 ? (
            <p className="text-sm text-gray-500">No notes yet. <a href="/projects/new" className="text-[#E91E63] underline">Create one.</a></p>
          ) : (
            <div className="space-y-2">
              {m.recentNotes.map(n => (
                <a key={n.id} href={`/projects/${n.id}`} className="block py-2 border-b border-gray-50 last:border-0 hover:bg-gray-50 -mx-2 px-2 rounded">
                  <div className="text-sm font-medium text-gray-900">{n.project_name}</div>
                  <div className="text-xs text-gray-500 mt-0.5 line-clamp-1">{n.objective}</div>
                </a>
              ))}
            </div>
          )}
...
function StatCard({ value, label, sub, highlight }) {
  return (
    <div className={`rounded-lg border p-5 ${highlight ? 'bg-[#E91E63] border-[#E91E63]' : 'bg-white border-gray-200'}`}>
