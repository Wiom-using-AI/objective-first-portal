...
export default function SubmitPage() {
  ...
  if (submitted) {
    return (
      <div className="max-w-2xl mx-auto text-center py-16">
        ...
        <p className="text-[#555555] mt-2 max-w-md mx-auto">
  ...
  return (
    <div className="max-w-4xl space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-[#102A43]">FL Project &amp; Objective Submission</h1>
        <p className="text-[#555555] mt-1">Blind baselining — complete this independently, without checking with anyone else.</p>
      </div>

      {/* Instructions */}
      <div className="bg-[#EAF1F8] border-l-4 border-[#1F6FB2] rounded-r-lg p-4">
        <p className="text-xs font-semibold text-[#1F6FB2] uppercase tracking-wide mb-1">Instructions</p>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-5">
          <h3 className="text-sm font-semibold text-[#102A43] uppercase tracking-wide mb-4">About You</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-[#555555] mb-1">Your Name</label>
              <input
                type="text" value={name} onChange={e => setName(e.target.value)}
                placeholder="Enter your full name"
                className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#1F6FB2] focus:border-transparent text-sm"
              />
            </div>
            <div>
              <label className="block text-sm font-medium text-[#555555] mb-1">Function</label>
              <select
              ...
              {func === '__custom__' && (
                <input
                  ...
                  className="w-full mt-2 px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#1F6FB2] focus:border-transparent text-sm"
                />
              )}
            </div>
          </div>
        ...
        <div className="space-y-4">
          <h3 className="text-sm font-semibold text-[#102A43] uppercase tracking-wide">Your Projects</h3>

          {projects.map((p, i) => (
            <div key={i} className="bg-white rounded-lg border border-gray-200 p-5 space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-xs font-semibold text-[#1F6FB2] uppercase tracking-wide">Project {i + 1}</span>
                {projects.length > 1 && (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-medium text-[#555555] mb-1">Project Name</label>
                  <input
                ...
                <div>
                  <label className="block text-xs font-medium text-[#555555] mb-1">Success Metric (current &rarr; target)</label>
                  <input
              ...
              <div>
                <label className="block text-xs font-medium text-[#555555] mb-1">Objective (outcome, 1 sentence)</label>
                <textarea
              ...
              <div className="flex items-center gap-3">
                ...
                {p.is_cross_fl && (
                  <input
                    ...
                    className="flex-1 px-3 py-1.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#1F6FB2] focus:border-transparent text-xs"
                  />
                )}
              </div>
            </div>
          ...
          <button type="button" onClick={addRow} className="w-full py-3 border-2 border-dashed border-gray-300 text-[#555555] rounded-lg hover:border-[#1F6FB2] hover:text-[#1F6FB2] transition-colors text-sm font-medium">
            + Add Another Project
          </button>
        </div>
        ...
        <div className="flex justify-end pt-2">
          <button
            ...
            className="px-8 py-3 bg-[#102A43] text-white rounded-lg hover:bg-[#1F6FB2] transition-colors font-semibold text-sm disabled:opacity-50"
          >
