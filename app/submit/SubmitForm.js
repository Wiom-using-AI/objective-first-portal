              ...
              <div className="flex gap-2">
                {ROLE_OPTIONS.map(r => (
                  <button key={r} type="button" onClick={() => setRole(r)}
                    className={`flex-1 py-2 rounded-lg text-sm font-medium transition-all ${
                      role === r ? 'bg-[#E91E63] text-white' : 'bg-gray-100 text-gray-500 hover:bg-gray-200'
                    }`}>
                    {r}
                  </button>
                ))}
                <button type="button" onClick={() => setRole(role === '__custom__' ? '' : '__custom__')}
                  className={`px-3 py-2 rounded-lg text-sm font-medium transition-all ${
                    role === '__custom__' ? 'bg-[#E91E63] text-white' : 'bg-gray-100 text-gray-500 hover:bg-gray-200'
                  }`}>
                  Other
                </button>
              </div>
