  ...
  return (
    <div className="max-w-3xl space-y-6">
      <div>
        ...
        <h1 className="text-2xl font-bold text-[#102A43] mt-2">New Objective Note</h1>
        <p className="text-sm text-[#555555] mt-1">One page, fixed format — the single source of truth for a project.</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-lg border border-gray-200 divide-y divide-gray-100">
        {fields.map(f => (
          <div key={f.key} className="flex">
            <div className="w-1/3 bg-[#EAF1F8]/50 px-5 py-4 flex flex-col justify-center">
              <label className="text-sm font-semibold text-[#102A43]">{f.label}</label>
        ...
        <div className="px-5 py-4 flex justify-end gap-3">
          <a href="/projects" className="px-4 py-2 border border-gray-300 text-[#555555] rounded-lg hover:bg-gray-50 text-sm">Cancel</a>
          <button type="submit" disabled={loading} className="px-6 py-2 bg-[#102A43] text-white rounded-lg hover:bg-[#1F6FB2] transition-colors font-semibold text-sm disabled:opacity-50">
