    ...
    fetch('/api/submissions').then(r => r.json()).then(data => {
      ...
      for (const s of data) {
        if (!grouped[s.submitter_name]) {
          grouped[s.submitter_name] = {
            name: s.submitter_name,
            email: s.submitter_email,
            role: s.submitter_role,
            function: s.submitter_function,
            projects: [],
          };
        }
            ...
            <div key={i} className="bg-white rounded-lg border border-gray-200 p-5">
              <div className="flex items-center gap-3 mb-3">
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  {person.role && (
                    <span className="text-xs bg-gray-100 text-gray-600 px-2 py-0.5 rounded">{person.role}</span>
                  )}
                  {person.function && (
                    <span className="text-xs bg-pink-50 text-[#E91E63] px-2 py-0.5 rounded">{person.function}</span>
                  )}
                </div>
