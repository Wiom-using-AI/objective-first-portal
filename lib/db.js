...
function migrate() {
  ...
  // Check if we have the old schema (user_id based) and need to reset
  const tableInfo = d.prepare("PRAGMA table_info(submissions)").all();
  const hasOldSchema = tableInfo.length > 0 && tableInfo.some(c => c.name === 'user_id');

  if (hasOldSchema) {
    // Drop old tables and recreate — no production data to preserve yet
    d.exec(`
