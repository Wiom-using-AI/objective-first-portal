...
export async function scoreObjectivePair(projectNameA, projectNameB, flA, flB) {
  ...
  const projectLabel = sameName
    ? `"${projectNameA}"`
    : `"${projectNameA}" (Functional Head A's name) / "${projectNameB}" (Functional Head B's name)`;

  const prompt = `You are an objective-alignment scorer for a company called Wiom.

Two Functional Heads independently wrote the objective for what appears to be the same project: ${projectLabel}.

Functional Head A (${flA.name}, ${flA.function || 'unknown function'}):
  Project name: ${projectNameA}
  Objective: ${flA.objective}
  Success Metric: ${flA.success_metric || 'not provided'}

Functional Head B (${flB.name}, ${flB.function || 'unknown function'}):
